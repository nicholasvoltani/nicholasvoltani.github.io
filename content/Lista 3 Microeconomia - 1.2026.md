...
Resultados importantes para a teoria do consumidor, assim como para a teoria da firma, são o lema de Shephard e a identidade de Roy.[^1]

O lema de Shephard associa a [[Demanda Hicksiana|demanda hicksiana]] $\vec{h}(\vec{p}, \bar{u})$ com a [[Função Dispêndio|função dispêndio]] $e(\vec{p}, \bar{u})$: a trajetória da demanda hicksiana é descrita pela variação do dispêndio com relação aos *preços*. Formalmente:
$$
...
# Ex 1
Através da função utilidade de Cobb-Douglas $U(x_{1},x_{2}) = x_{1}^{\alpha}x_{2}^{1-\alpha}$, e supondo vetor de preços $\vec{p}$ e restrição orçamentária $w$, demonstre que vale a equação de Slutsky. Para isso, resolva os seguintes passos:
1) Obtenha as demandas walrasianas $x^*(\vec{p}, w)$ e $y^*(\vec{p},w)$ pelo problema de maximização de utilidade
2) Com as demandas walrasianas, obtenha a função de utilidade indireta $v(\vec{p}, w) = U(x^*(\vec{p},w), y^*(\vec{p},w))$, a ser denotada como $\bar{u} \in \mathbb{R}$
3) Obtenha as demandas hicksianas (também chamadas de demandas *compensadas* de Hicks) $h_{x}(\vec{p}, \bar{u})$ e $h_{y}(\vec{p}, \bar{u})$. Para isso, utilize a identidade de Roy para cada bem $i$:
$$
...
(A) Nenhuma evacuação é necessária e nada é feito.

(B) Uma evacuação é feita, que é desnecessária

(C) Uma evacuação é feita, que é necessária.

(D) Nenhuma evacuação é feita e uma inundação causa um desastre.

Suponha que uma agência seja indiferente entre o resultado certo de B e a loteria de A com probabilidade $p$ e D com probabilidade $1-p$, e entre o resultado certo C e a loteria de A com probabilidade $q$ e D com probabilidade $1-q$.[^3] Suponha, também, que ela prefira A a D, e que $p \in(0, 1)$ e $q\in(0,1)$. Assuma que as condições do teorema da utilidade esperada sejam satisfeitas.

(a) Construa uma função de utilidade na forma de utilidade esperada para a agência.
(b) Considere dois critérios diferentes de política:
	1) Critério 1: Esse critério resultará em uma evacuação em $90\%$ dos casos nos quais a inundação ocorrerá e uma evacuação desnecessária em $10\%$ dos casos nos quais nenhuma inundação ocorre.
	2) Critério 2: Esse critério é mais conservador. Resulta em uma evacuação em $95\%$ dos casos nos quais a inundação ocorrerá e uma evacuação desnecessária em $15\%$ dos casos nos quais nenhuma inundação ocorre.
...
Temos a seguinte matriz de decisão:[^4]
...
(Exercício 6.C.1) Mostre que se o seguro **não** é atuarialmente justo (de modo que $q>\pi$), então o indivíduo **não** irá se segurar completamente. 
...
u(x) = 1 - e^{-\alpha x}
$$
possui aversão absoluta ao risco $r_{A}(x)$ constante. (Em inglês, diz-se que ela apresenta CARA — *constant absolute risk aversion*.) O que significa $\alpha$ aumentar?
...
$$
possui aversão relativa ao risco $r_{r}(x)$ constante. (Em inglês, diz-se que ela apresenta CRRA — *constant relative risk aversion*.) O que significa $\rho$ aumentar?
...
[^1]: Suas demonstrações são mais convolutas. Para o leitor interessado: suas demonstrações giram em torno do *envelope theorem*, e recomendo singelamente minha demonstração destes resultados em [Envelope Theorem — nicholasvoltani.github.io](https://nicholasvoltani.github.io/Envelope-Theorem)
[^2]: Isso é relevante! No que tange à demanda compensada $\vec{h}(\vec{p}, \bar{u})$, ela é uma demanda *para um dado nível de utilidade* $\bar{u}$; a restrição orçamentária empregada para alcançar este nível de utilidade $\bar{u}$, naturalmente, *depende deste nível de utilidade* $\bar{u}$ (além dos preços)! Não cometam os mesmos erros de cálculo que eu!
[^3]: Tradução: Essa agência possui as seguintes preferências: $B \sim \left(p A + (1-p) D\right)$; $C \sim \left(q B + (1-q) D\right)$; e $A \succ D$. 
[^4]: O símbolo $\lnot$ significa "negação" (em LaTeX: `\lnot`). A notação $\lnot A$ lê: "não-$A$".
[^5]: Não, não é $3.1415 \dots$
[^6]: Um "sinistro" ocorre quando um seguro é acionado devido a uma perda a ser indenizada. O pagamento de um sinistro se chama "indenização".
[^7]: Pode-se verificar isso com L'Hôpital, e lembrando que $\frac{ \partial x^{-\rho}}{ \partial \rho } = -\ln(x) x^\rho$. 
