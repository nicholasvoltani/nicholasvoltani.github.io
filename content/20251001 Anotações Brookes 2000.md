...
$$
E_{old} \mapsto \frac{1}{\tau} E_{old} \eqqcolon E_{new}
$$
...
![[Pasted image 20251001100617.png|500]]
...
- Efeito *output*: um aumento da eficiência $\tau$ faz com que a produção, para dados níveis de inputs $(R, E)$, seja $Y^* = f(R, \tau E) > \bar{Y}$ 
...
- $(-\infty, 0\%)$: Há *superconservação* de energia, um efeito desproporcional de poupança maior do que o esperado. ${\color{red} E_{new}} < \tau E_{old}<E_{old}$ 
- $0\%$: Há um aumento de eficiência tal que **o produto se mantém o mesmo** — há uma diminuição de input de energia *pari passu*: supondo que $(\tau \downarrow \iff Eff \uparrow)$ \[**i.e. este $\tau$ aqui usado é o inverso do $\tau$ utilizado por Saunders acima**\], $\tau E_{old} = {\color{red} E_{new}} < E_{old}$
- $(0\%, 100\%)$: A diminuição de input de energia é menor que o aumento da energia: $\tau E_{old} < {\color{red} E_{new}} < E_{old}$
- $100\%$: Não há diminuição de input de energia: $\tau E_{old} < {\color{red} E_{new}} = E_{old}$
- $(100\%, \infty)$: Há *aumento* do input de energia: $\tau E_{old} < E_{old} < {\color{red} E_{new}}$

Ou seja, há uma comparação entre a energia poupada *esperada* — $(1-\tau) E_{old}$ — e a energia poupada *efetiva* — $E_{old} - E_{new}$. 

$$
\text{Rebound} = 1 - \frac{1}{1-\tau} \frac{E_{old} - E_{new}}{E_{old}} \coloneqq \frac{\tilde{\tau} - \tau}{1 - \tau}
$$
...
- SAUNDERS, Harry. Theoretical foundations of the rebound effect. _In_: EVANS, Joanne; HUNT, Lester C. (Orgs.). **International handbook on the economics of energy**. Cheltenham: Edward Elgar, 2009.
