...
[^1]: Tome-se algum $N_{0}$ nível de emprego referência, **partindo do equilíbrio** ($Z(N_{0}) = D(N_{0})$). Então, para qualquer variação $\epsilon \in \mathbb{R}$ em torno de $N_{0}$, teremos, *ex hypothesi* de Lei de Say, que $Z(N_{0}+\epsilon) = D(N_{0}+\epsilon)$. Portanto, as funções devem corresponder à mesma função!