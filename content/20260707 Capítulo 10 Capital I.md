...
# Livro I, Capítulo 10: O conceito de mais-valor relativo
## Divisão da jornada de trabalho em trabalho necessário e excedente
Dada uma [[Jornada de Trabalho]] $T$ constante, ela diferencia-se em [[Tempo de Trabalho Necessário]] $t_{N}$ e [[Tempo Excedente de Trabalho]] $t_{E}$, a depender do [[Valor da Força de Trabalho]] $v$. Neste caso,
> "Ao prolongamento do mais-trabalho corresponderia o encurtamento do trabalho necessário, ou, em outras palavras, a parte do tempo de trabalho que o trabalhador até agora utilizava para si mesmo seria convertida em tempo de trabalho para o capitalista [que o emprega]. A mudança [das proporções da jornada de trabalho] estaria não na duração da jornada de trabalho, mas em sua divisão em trabalho necessário e mais-trabalho." [@Marx2017, p. 387]

## Componentes do valor da força de trabalho/tempo necessário
Marx já acrescenta no parágrafo seguinte:
> "Por outro lado, com dada grandeza da jornada de trabalho e dado valor da força de trabalho, a grandeza do mais-trabalho é evidentemente dada." [@Marx2017, p. 387]

Diferencia entre a *produção* da [[Força de Trabalho]] e a *reprodução* de seu [[valor]]:
> "O valor da força de trabalho, isto é, o tempo de trabalho requerido para sua produção, determina o tempo de trabalho necessário para a reprodução de seu valor. [...] Com o valor de seus meios de subsistência está dado o valor de sua [do trabalhador] força de trabalho, e com o valor de sua força de trabalho está dado a grandeza de seu tempo de trabalho necessário." [@Marx2017, p. 388]
...
Dessa forma, aumentar o tempo excedente — logo, aumentar a obtenção de mais-valor — pode ocorrer ao aumentar a jornada de trabalho — i.e. [[Mais-Valor Absoluto]] — e/ou ao diminuir o tempo necessário dessa jornada — i.e. [[Mais-Valor Relativo]]. 
> "Ao mais-valor obtido pelo prolongamento da jornada de trabalho chamo de mais valor absoluto; ao mais-valor que, ao contrário, deriva da redução do tempo de trabalho necessário e da correspondente alteração na proporção entre as duas partes da jornada de trabalho chamo de mais-valor relativo." [@Marx2017, p. 390]

Como o valor da força de trabalho depende do valor das mercadorias que a compõem — assim como em qualquer [[processo de produção capitalista]] —, aumentos de forças produtivas somente diminuem seu valor se afetarem *os setores que produzem suas mercadorias "componentes"*, i.e. quando há uma queda no [[Tempo de Trabalho Socialmente Necessário]] para a produção destas mercadorias, e, portanto, uma diminuição *pro tanto* do valor da força de trabalho. 
> "Em contrapartida, nos ramos de produção que não fornecem nem meios de subsistência nem meios de produção para fabricá-los, a força produtiva aumentada deixa intocado o valor da força de trabalho." [@Marx2017, p. 390]
...
$$
\begin{cases}
\bar{c} &= 6\ \frac{\text{pence}}{\text{peça}} \\
\bar{v}_{0} &= \frac{6\ \frac{\text{pence}}{\text{hora}} \cdot 12\ \text{horas}}{12\ \text{peças}}= \frac{6\ \text{sh.}}{12\ \text{peças}} = \frac{1}{2}\ \frac{\text{sh.}}{\text{peça}} \\
&= 6\ \frac{\text{pence}}{\text{peça}}
\end{cases}
$$
...
### Analisando tempos necessário e excedente
Suponha-se que $T = t_{N} + t_{E}$, onde $t_{N} = 10$h e $t_{E} = T - t_{N}= 2$h. Portanto, vide acima,
- $T=12$h custa $6$ sh.
	- $t_{N} = 10$h custa $5$ sh. (= valor diário da força de trabalho)
	- $t_{E}=2$h custa $1$ sh.
...
- $\bar{c}$: valor de meios de produção por peça
- $T$: jornada de trabalho, em horas
- $\tilde{v}$: valor *por hora* da força de trabalho
	- $v=\tilde{v}T$: valor da força de trabalho (trabalho *necessário*) em uma jornada de trabalho
- $m^{\prime}$: taxa (média) de mais-valor
...
(p-1)v + m^{\prime}pv
$$
Sua taxa de mais-valor efetiva é, agora,
$$
m^{\dagger} = \frac{m^{\prime}pv + (p-1)v}{v} = m^{\prime} p + p-1
$$

Nos casos acima, tivemos que $p=2$, para o qual obtemos
$$
m^{\dagger} = 2 m^{\prime} + 1
$$
Para $m^{\prime}=20\%$, temos
$$
m^{\dagger} = \frac{1}{5} + 1 = \frac{7}{5}
$$
como esperado.
...
Caso ele venda seu produto ao "valor social", ele obterá
$$
\frac{pN}{N} (c+\bar{v}\Delta t) = pc + p \bar{v}\Delta t
$$
Disso ele recupera $pc$ transferido pelo capital constante, e paga o valor da força de trabalho despendida $\nu\bar{v}\Delta t$. Resta-lhe, portanto, um excedente de
$$
p (1-\nu) \bar{v} \Delta t
$$
...
$$
Seu tempo excedente aparece-lhe, portanto, como
$$
\tilde{t}_{E} = T - \tilde{t}_{N} = \frac{pT-t_{N}}{p}
$$
Comparado com a média, ele obtém um equivalente de tempo excedente de $\Delta t_{E}$, que se obtém da seguinte forma:
$$
\begin{align*}
\Delta t_{E} &\equiv  \tilde{t}_{E} - t_{E} \\ 
&= T - \frac{t_{N}}{p} \pm t_{N} - t_{E} \\
&= t_{N}\left( 1-\frac{1}{p} \right) \\
&= \frac{p-1}{p}\ t_{N}
\end{align*}
$$
...
$$
\begin{align*}
m^{\dagger} &\equiv \frac{\tilde{t}_{E}}{\tilde{t}_{N}} \\
&= \frac{pT-t_{N}}{t_{N}} \\
